...
const BinomialChart = {
  ...
  update(state) {
    ...
    const bandData = k !== null
      ? [{ start: mode === 'eq' ? k : k, end: mode === 'eq' ? k : n }]
      : [];

    const band = this.svg.select('.band')
      .selectAll('rect')
      .data(bandData);
    ...
    band.enter()
      ...
      .attr('x', d => x(d.start))
      .attr('y', 0)
      .attr('width', d => mode === 'eq'
        ? x.bandwidth()
        : x(d.end) + x.bandwidth() - x(d.start)
      )
    ...
    this.svg.select('.x-axis').call(d3.axisBottom(x).tickValues(
    ...
    bars.enter()
      ...
      .attr('fill', d => this._isHighlighted(d.k, k, mode)
    ...
    return mode === 'eq'
      ? Binomial.pmf(k, n, p)
      : Binomial.probabilityGreaterThan(k - 1, n, p);
  }
};