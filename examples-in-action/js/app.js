...
function updateDescription(state, prob) {
  ...
  const pvalue =
    prob >= 0.05
      ? 'Étant donné que ' +
        katex.renderToString(`p \\geq 0.05`, { throwOnError: false }) +
        " : le résultat obtenu n'est pas extraordinaire dans le monde de " +
        katex.renderToString(`H_0`, { throwOnError: false }) +
        ". Nous n'avons pas amassé suffisament de preuves pour basculer vers " +
        katex.renderToString(`H_1`, { throwOnError: false }) +
        ' et conclure que la pièce est truquée : nous devons rester dans ' +
        katex.renderToString(`H_0`, { throwOnError: false }) +
        ". Soit notre pièce n'est pas truquée en faveur du côté face, soit elle l'est mais nous n'en avons pas la preuve."
      : 'Étant donné que ' +
        katex.renderToString(`p \\lt 0.05`, { throwOnError: false }) +
        ' : le résultat obtenu est extraordinaire dans le monde de ' +
        katex.renderToString(`H_0`, { throwOnError: false }) +
        '. Si la pièce était vraiment équilibrée, observer ce résultat aurait été trop peu probable. Avec un niveau de preuve suffisant, on bascule vers ' +
        katex.renderToString(`H_1`, { throwOnError: false }) +
        ' : on peut raisonnablement croire que notre est pièce est truquée en faveur du côté face.';
