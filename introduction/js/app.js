...
function updateDescription(mu, sigma, threshold) {
  ...
  const conclusion =
    prob >= 0.05
      ? `Comme p ≥ 0.05, ce résultat est alors ordinaire dans le monde où $H_0$ est vraie : le fait qu'un chêne mesure ${threshold} m n'est pas un fait extraordinaire, observer ce résultat ou un résultat plus extrême n'est pas improbable. Il n'y a pas assez de preuves pour basculer vers $H_1$ : Grace ne peut pas rejetter $H_0$ et conclure que l'arbre est un hêtre, elle doit se résigner à conclure que l'arbre est un chêne.`
      : `Comme p < 0.05, ce résultat est alors extraordinaire dans le monde où $H_0$ est vraie : si l'arbre était vraiment un chêne, observer ce résultat (mesurer ${threshold} m) ou un résultat aussi extrême aurait été trop peu probable. On considère qu'on a atteint un niveau de preuves suffisant pour rejetter $H_0$ et basculer vers $H_1$ : Grace pourra conclure que l'arbre est un hêtre.`;
